/*
Класс управляет охотниками и ульями.

*/
let HuntersManager = function (json_params)
{
	this.Scene = scene;
	this.HuntersArray = [];
	this.BeehiveArray = [];
	...
	for (p_name in json_params_names)
	{
		if(json_params[p_name])
		{
			this[p_name] = json_params[p_name];
		} else
		{
			throw new Error("Have no mandatory parameter: " + p_name);
		}
	}
...
*/
HuntersManager.prototype.update = function (mul)
{
	for(let i=0; i< this.ObjectsArray.length; i++)
	{	
		let distTo = this.LocalUserMesh.position.distanceTo(this.ObjectsArray[i].position);
...
*/
HuntersManager.prototype.resetObjectsPositions = function ()
{
	for(let i=0; i< this.ObjectsArray.length; i++)
	{
...
HuntersManager.prototype.createObjects = function ()
{
...
*/
HuntersManager.prototype.getRandomMinusMult = function ()
{
	let multip = -1;
...
HuntersManager.prototype.deleteObjects = function ()
{
