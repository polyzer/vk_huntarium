...
var Menu = function (json_params)
{
	...
	var startexture = new THREE.ImageUtils.loadTexture("./images/bg_1_1.png");
...
*/	
	...
	this.Inputs.Banner_Text = {};
	this.Inputs.Banner_Text.ObjHTML = document.createElement("div");
	this.Inputs.Banner_Text.ObjHTML.id = "Banner_Text";

	this.Inputs.Banner_Text.Obj3DCSS =  new THREE.CSS3DObject(this.Inputs.Banner_Text.ObjHTML);
	this.Inputs.Banner_Text.Obj3DCSS.position.x = 0;
	this.Inputs.Banner_Text.Obj3DCSS.position.y = 170;
	this.CSSScene.add(this.Inputs.Banner_Text.Obj3DCSS);
...
*/
Menu.prototype.onTrainingButtonClick = function ()
{
	$("#training_page_div").show();
};
/*Обработчик нажатия клавиши закрытия обучения
*/
Menu.prototype.onTrainingCloseButtonClick = function ()
{
	$("#training_page_div").hide();
};
/*При нажатии на клавишу вызова друзей*/
Menu.prototype.onOpenCallFriendsListButtonClick = function ()
{
	VK.callMethod("showInviteBox");
};
/*Создаёт движущиеся объекты!*/
Menu.prototype.createFlyingObjects = function ()
{
	this.FlyingObjects = [];
	for (var i=0; i<GAME_CONSTANTS.FLYING_OBJECTS.NEAREST_OBJECTS_COUNT; i++)
	{
		...
		el.position.y = GAME_CONSTANTS.FLYING_OBJECTS.FLYING_MIN_HEIGHT_START_POSITION*Math.random()-200;
		el.position.z = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()-1.5);
		el.position.x = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()*2-1);

		this.FlyingObjects.push(el);
		this.Scene.add(el);
	}

	for(var i=0; i < GAME_CONSTANTS.FLYING_OBJECTS.FARTHER_OBJECTS_COUNT; i++)
	{
		var el = new THREE.Mesh(
		...
		el.position.z = GAME_CONSTANTS.FLYING_OBJECTS.FLYING_MIN_HEIGHT_START_POSITION*Math.random()-400;
		el.position.y = GAME_CONSTANTS.FLYING_OBJECTS.FARTHER_OBJECTS_DISTANCE + 100*(Math.random()-0.5);
		el.position.x = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()*2-1);
...
Menu.prototype.controlFlyingObjects = function ()
{
	for(var i=0; i< GAME_CONSTANTS.FLYING_OBJECTS.NEAREST_OBJECTS_COUNT; i++)
	{		
		if(this.FlyingObjects[i].position.z >= GAME_CONSTANTS.FLYING_OBJECTS.FLYING_MAX_HEIGHT)
		{
			this.FlyingObjects[i].position.z = GAME_CONSTANTS.FLYING_OBJECTS.FLYING_MIN_HEIGHT_START_POSITION*Math.random()-900;
			this.FlyingObjects[i].position.y = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()-1.5);
			this.FlyingObjects[i].position.x = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()*2-1)*3;
		} else
		{
	...
	for(var i= GAME_CONSTANTS.FLYING_OBJECTS.NEAREST_OBJECTS_COUNT; i<(GAME_CONSTANTS.FLYING_OBJECTS.NEAREST_OBJECTS_COUNT+GAME_CONSTANTS.FLYING_OBJECTS.FARTHER_OBJECTS_COUNT); i++)
	{		
		if(this.FlyingObjects[i].position.z >= GAME_CONSTANTS.FLYING_OBJECTS.FARTHER_FLYING_MAX_HEIGHT)
		{
			this.FlyingObjects[i].position.z = GAME_CONSTANTS.FLYING_OBJECTS.FLYING_MIN_HEIGHT_START_POSITION*Math.random()-900;
			this.FlyingObjects[i].position.y = GAME_CONSTANTS.FLYING_OBJECTS.FARTHER_OBJECTS_DISTANCE + 300*(Math.random()-0.5);
			this.FlyingObjects[i].position.x = GAME_CONSTANTS.FLYING_OBJECTS.FlYING_RADIUS*(Math.random()*2-1)*3;
		} else
		{
...
Menu.prototype.onMakeStreamRequestButtonClick = function()
{
	makeRightStreamRequest({
		constraints:{video: true, audio: true},
		onsuccess: this.onsuccessBF, 
...
/*При создании потока.*/
Menu.prototype.onsuccess = function (streamp) {
...
Menu.prototype.onConnectionOpen = function ()
{
...
Menu.prototype.initMultiRoomMode = function ()
{
...
Menu.prototype.initSingleRoomMode = function ()
{
	this.RoomID = DEFAULT_ROOM_ID;
};
/*Делает запрос на 
 */
Menu.prototype.setRoomsList = function ()
{
 ...
 * Доступно только в MULTI_ROOMS режиме!!!
	...
	* }
*/
Menu.prototype.createSelectOptionsByRoomsList = function (json_params)
{
...
Menu.prototype.onRoomSelect = function ()
{
...
/* Отправляет запросы и обрабатывает ответы.
	...
	* }
*/
Menu.prototype.sendServerRequest = function (json_params)
{
...
Menu.prototype.hasSelectedRoom = function ()
{
	if(window.GLOBAL_OBJECTS.Room){
		alert("OK");
		return true;
	}
};

Menu.prototype.canStart = function ()
{
 ...
 */
Menu.prototype.onStartButtonClick = function ()
{
	if(this.canStart() === true)
	{
		...
		{
			GLOBAL_OBJECTS.setGame(new Game({
				camera: this.Camera,
...
Menu.prototype.onNicknameEntering = function ()
{
...
Menu.prototype.onNicknameMaxLength = function ()
{
...
Menu.prototype.onErrorNicknameEntering = function ()
{
	this.Inputs.Nickname.ObjHTML.value = this.Inputs.Nickname.ObjHTML.value.slice(0, this.Inputs.Nickname.ObjHTML.value.length-1);
	this.Inputs.Nickname.ObjHTML.style.border = "5px solid red";
};

Menu.prototype.onRightNicknameEntering = function ()
{
